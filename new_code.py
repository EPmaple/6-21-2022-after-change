        ...
        if ('<@&887894985398157363>') in message.content or (
            ...
            if len(message.raw_mentions) != 0:
                ...
                await message.channel.send(
                    f'Woah! It is a slime!  (ﾉ>ω<)ﾉ  Klee has counted {AGE_members[member_id]} slimes for {id_name[member_id]}!'
                )
...
async def doubleping(ctx, *, member):
  try:
    if ctx.channel.id == 887894832708730881 or ctx.channel.id == 887967982356148254:
        ...
        await ctx.send(
            f'Klee has subtracted a slime from {id_name[member_id]}! The number of slimes {id_name[member_id]} has summoned has gone from {int(AGE_members[member_id])+1} to {AGE_members[member_id]}'
        )
        return
...
async def add(ctx, number, *, username):
  try:
    if ctx.channel.id == 887894832708730881 or ctx.channel.id == 887967982356148254:
        ...
        await ctx.send(
            f'Klee has added a slime to {id_name[member_id]}! The number of slimes {id_name[member_id]} has summoned has gone from {original} to {AGE_members[member_id]}'
        )
